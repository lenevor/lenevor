...
namespace Syscodes\Components\ThrottleLimiter;
...
/**
 * Allows the throttle limiter in request and api's.
 */
class ThrottleLimiter
{
    ...
    /**
     * Constructor. Create a new Throttle limiter class instance.
     * 
    ...
    public function tooManyAttempts($key, $maxAttempts): bool
    {
        if ($this->attempts($key) >= $maxAttempts) {
            if ($this->cache->has($this->cleanThrottleLimiterKey($key).':timer')) {
                return true;
            }
    ...
    {
        $key = $this->cleanThrottleLimiterKey($key);
    ...
    /**
     * Clean the throttle limiter key from unicode characters.
     * 
     * @param  string  $key
     * 
     * @return string
     */
    public function cleanThrottleLimiterKey($key): string
    {
